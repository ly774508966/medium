...
var Shader = function () {
	function Shader(options, vertexShader, fragmentShader) {
		...
		for (var key in this.customUniforms) {
			var uniform = this.customUniforms[key];
			if (uniform.type === 't') {
		...
		for (var _uniform in this.uniforms) {
			this.uniforms[_uniform].location = gl.getUniformLocation(this.program, _uniform);
		}
	...
	_createClass(Shader, [{
		...
		value: function setUniforms(modelViewMatrix, projectionMatrix, modelMatrix) {
			...
			for (var uniform in this.customUniforms) {
				...
				var value = this.uniforms[uniform].value;
